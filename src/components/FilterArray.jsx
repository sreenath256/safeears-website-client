...
const FilterArray = ({ list, handleClick }) => {
  ...
  return (
    <div className="flex justify-between gap-2 font-semibold bg-white p-3 text-gray-500 my-2  shadow rounded-md text-sm">
      {list.map((status) => (
        <p
          key={status}
          className={
            activeStatusFilter === status
              ? "bg-gray-100 rounded px-2 py-1 text-secondary cursor-pointer"
              : "admin-status"
          }
          onClick={() => {
